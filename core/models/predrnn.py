...
class RNN(nn.Module):
    ...
    def forward(self, tmp_frames_tensor):
        ...
        for i in range(self.num_layers):
            # zeros = torch.zeros(int(batch), self.num_hidden[i], int(height), int(width)).to(self.configs.device)
            zeros = torch.zeros([batch, self.num_hidden[i], height, width]).to(self.configs.device)
            h_t.append(zeros)
            c_t.append(zeros)

        memory = torch.zeros([batch, self.num_hidden[0], height, width]).to(self.configs.device)
        # memory = torch.zeros(int(batch), self.num_hidden[0], int(height), int(width)).to(self.configs.device)
        ...
        # print ('next_frames', len(next_frames), next_frames[0].size())
