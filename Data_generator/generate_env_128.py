...
pic_dir = "../Data/2D_map/Map_pictures"
...
def generate_2D_map(size=100, num_config=16000, maz_id=0):
	...
	obs_num = 15#12
	obs_pos = np.random.randint(low=11, high=117, size=(obs_num, 2)) 
	...
	def generate_sg(nc, space):
		...
		for i in range(nc):
			...
			while True:
				s_pos = np.random.randint(low=0, high=127, size=(2)) 
				g_pos = np.random.randint(low=0, high=127, size=(2)) 
				if space[s_pos[0], s_pos[1]] == 255 or space[g_pos[0], g_pos[1]] == 255 or np.sum(np.absolute(s_pos - g_pos)) < 150: 
					continue
				flag = True
...
def generate_2D_dataset():
	...
	total_obs = []
	for i in range(10):
		maz, s_g_confg, obs = generate_2D_map(128, 2000, i)
	...
	train_config = total_config[:,:1600,:]
	test_seen_config = total_config[:,1600:,:]

	test_unseen_config = []
	for i in range(10, 12):
		maz, s_g_confg, obs = generate_2D_map(128, 400, i)
		total_maz.append(maz)
